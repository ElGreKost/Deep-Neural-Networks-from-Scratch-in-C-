...
public:
    ...
    int Add_To_Tail(Base_Link *element) {curr = NULL; return Add_Node(element);}
    ...
    inline Base_Link *Curr() {return !curr ? NULL : curr -> element;}
...
public:
    ...
    LList *In_Links() { return &in_links; }

    LList *Out_Links() { return &out_links; }
...
int Disconnect(Base_Node *from_node, Base_Node *to_node) {
    ...
    for (int i = 0; i < out_links->Count(); ++i) { // for each output link
        if (out_links->Curr()->Out_Node() == to_node) {flag = 1; break;}
        out_links->Next();
    }
...
public:
    ...
    virtual char *Get_Name () { static char name[] = "FEED_FORWARD_NODE"; return name; }
};
...
public:
    ...
    virtual char *Get_Name() {static char name[] = "BASE_NETWORK"; return name;}
};
